    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                serverMessage = errorData.message || 'Không thể tạo PDF.';
            } catch (e) {
                // If JSON parsing fails, the body might be plain text or empty
                const errorText = await response.text();
                serverMessage = errorText || response.statusText || 'Lỗi không xác định từ server.';
                errorData = { message: serverMessage }; // Create a consistent error object
            }
            
            console.error('PDF Generation API Error:', errorData);
            throw new Error(`Lỗi từ server: ${serverMessage}`);
        }
